...
///
/// Sends a message containing an embed as the reply to the command in the channel the command \
/// was used in. Also lets you optionally add a normal message along with the embed. To send a \
/// normal message without an embed use the `say` command instead.
///
...
///
/// Makes the bot send the given message as the reply to the command in the channel the command \
/// was used in. To send an embed use the `embed` command instead.
///
/// Makes sure to handle escaped characters like `\n` properly.
#[poise::command(slash_command, guild_only)]
pub async fn say(
